function Arete(crayon,noeud1,noeud2,direction,couleurs,epaisseures,nom,coordonnee_nom,D) {
	...
	this.tracer = function(ponderation,oriente){
		if(this.noeud1.isVisible() && this.noeud2.isVisible()){
			...
			if(this.centrer || !oriente || this.direction == 0){
			...
			if(ponderation){
				...
				//on se prépare our écrire
				this.crayon.font = "14pt Verdana";
				this.crayon.textAlign="center";
				this.crayon.textBaseline="middle";

				//calcul de la taille du rectangle à netoyer pour pouvoir ecrire
				var h_texte = parseInt(this.crayon.font)+8;
				var l_texte = this.crayon.measureText(this.valeur+"/"+this.nom).width+4;

				this.crayon.fillRect(Math.round( this.coordonnee_nom *(x1+x2) / 2 ) - l_texte/2 , Math.round( this.coordonnee_nom *(y1+y2)/2) - h_texte/2 , l_texte , h_texte);
				this.crayon.fill();

				this.crayon.fillStyle = this.couleurs[this.ETAT];
				
				this.crayon.fillText(this.valeur+"/"+this.nom , Math.round( this.coordonnee_nom *(x1+x2) / 2 ) , Math.round( this.coordonnee_nom *(y1+y2)/2));
			}
